...
## Шаг 3. Запустить автоматическую установку

```bash
sudo bash scripts/server-setup.sh
```

Скрипт спросит три вещи (домен, email для сертификата, email администратора)
и дальше сделает всё сам:

1. поставит Docker, nginx, certbot, firewall;
2. создаст `.env` и сгенерирует секреты (пароль БД, ключи сессий);
